...
class UserViewModel {
  ...
  static parseCardData(user) {
    ...
    const instagramItemUpdateCount = nconf.get(
      "auths:instagram:itemUpdateCount"
    );
    const facebookItemUpdateCount = nconf.get("auths:facebook:itemUpdateCount");

    return []
      .concat(
        parseYoutubeCards(user.google, googleItemUpdateCount),
...
function parseFacebookCards(facebookData, facebookItemUpdateCount) {
  ...
  return facebookData.rawData.data
    .filter(
      item => item.privacy && item.privacy.value.toLowerCase() === "everyone"
    )
